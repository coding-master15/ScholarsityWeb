    ...
    function write_config($data) {
        ...
        if (is_writable($output_path)) {
            ...
            if (fwrite($handle, $new)) {
                $template_path_url = 'includes/configApp.php';

                $output_path_url = '../application/config/config.php';

                $file_url = file_get_contents($template_path_url);
                ...
                $new_url = str_replace("%BASEURL%", $str.$str1, $file_url);

                $handle = fopen($output_path_url, 'w+');
                @chmod($output_path_url, 0777);

                if (is_writable($output_path_url)) {

                    if (fwrite($handle, $new_url)) {
                        return true;
                    } else {
                        return false;
                    }
